 ...
 */
class RoundRobinContainerPoolBalancer(
  config: WhiskConfig,
...
//extends CommonLoadBalancer(config, feedFactory,controllerInstance) // avs added loadFeedFactory
  ...
  val schedulingState = RoundRobinContainerPoolBalancerState()(rrLbConfig)

  /**
   * Monitors invoker supervision and the cluster to update the state sequentially
   *
   * All state updates should go through this actor to guarantee that
   * [[RoundRobinContainerPoolBalancerState.updateInvokers]] and [[RoundRobinContainerPoolBalancerState.updateCluster]]
  ...
  override def publish(action: ExecutableWhiskActionMetaData, msg: ActivationMessage)(
    ...
    val chosen = if (invokersToUse.nonEmpty) {
      ...
      val stepSize = 1
      // avs --end
      logging.info(this,s"<avs_debug> <RoundRobinContainerPoolBalancer> <publish> Calling schedule for activation ${msg.activationId} for '${msg.action.asString}' ($actionType) ") // avs
      val invoker: Option[(InvokerInstanceId, Boolean)] = RoundRobinContainerPoolBalancer.schedule(
...
object RoundRobinContainerPoolBalancer extends LoadBalancerProvider {
  ...
  override def instance(whiskConfig: WhiskConfig, instance: ControllerInstanceId)(
    ...
    new RoundRobinContainerPoolBalancer(
      whiskConfig,
      instance,
 ...
 */
case class RoundRobinContainerPoolBalancerState(
  ...
  private var _managedStepSizes: Seq[Int] = RoundRobinContainerPoolBalancer.pairwiseCoprimeNumbersUntil(0),
  private var _blackboxStepSizes: Seq[Int] = RoundRobinContainerPoolBalancer.pairwiseCoprimeNumbersUntil(0),
  ...
  adaptiveLbConfig: RoundRobinContainerPoolBalancerConfig =
    loadConfigOrThrow[RoundRobinContainerPoolBalancerConfig](ConfigKeys.loadbalancer))(implicit logging: Logging) {
  ...
  def updateInvokers(newInvokers: IndexedSeq[InvokerHealth]): Unit = {
    ...
    if (oldSize != newSize) {
      _managedStepSizes = RoundRobinContainerPoolBalancer.pairwiseCoprimeNumbersUntil(managed)
      _blackboxStepSizes = RoundRobinContainerPoolBalancer.pairwiseCoprimeNumbersUntil(blackboxes)
 ...
 */
case class RoundRobinContainerPoolBalancerConfig(managedFraction: Double, blackboxFraction: Double, timeoutFactor: Int)
